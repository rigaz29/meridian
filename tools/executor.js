...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      maxPriceVolatility:  ["screening", "maxPriceVolatility"],
      ...
      trailingPeakConfirmTolerance:    ["management", "trailingPeakConfirmTolerance"],
      trailingDropConfirmTolerancePct: ["management", "trailingDropConfirmTolerancePct"],
      stopLossConfirmTolerancePct:     ["management", "stopLossConfirmTolerancePct"],
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        notifyDeploy({ pair: result.pool_name || args.pool_name || args.pool_address?.slice(0, 8), amountSol: args.amount_y ?? args.amount_sol ?? 0, amountX: result.amount_x, position: result.position, tx: result.txs?.[0] ?? result.tx, priceRange: result.price_range, binStep: result.bin_step, baseFee: result.base_fee, strategy: result.strategy }).catch(() => {});
      } else if (name === "close_position") {
        notifyClose({ pair: result.pool_name || args.position_address?.slice(0, 8), pnlUsd: result.pnl_usd ?? 0, pnlPct: result.pnl_pct ?? 0, feesUsd: result.fees_usd, initialValueUsd: result.initial_value_usd, minutesHeld: result.minutes_held, closeReason: result.close_reason, tx: result.close_txs?.[0] ?? result.txs?.[0] }).catch(() => {});
